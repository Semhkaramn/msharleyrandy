...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== AKTİVİTE MANUEL KİŞİ SAYISI ==========
    if waiting_for == 'activity_top_custom':
        from config import ACTIVITY_GROUP_ID
        ...
        if count < 1 or count > 100:
            await message.reply_text(
                "❌ Kişi sayısı 1-100 arasında olmalıdır.",
                parse_mode="HTML"
            )
            return

        # Kaydet
        success = await create_or_update_activity_settings(ACTIVITY_GROUP_ID, top_count=count)

        if success:
            context.user_data.pop('waiting_for', None)

            await message.reply_text(
                f"✅ Kişi sayısı <b>{count}</b> olarak ayarlandı!",
                parse_mode="HTML"
            )
